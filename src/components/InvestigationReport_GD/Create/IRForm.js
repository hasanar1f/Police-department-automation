...
import updateGDTimeline from '../../../axios/GD/updateTimeline';
import createInvestigationReportGD from '../../../axios/InvestigationReportGD/create';
	...
	const onSubmit = async data => {
		...
		if (irScanCopy.length < 1) {
			...
			await upload(
				...
				{
					directory: '/investigationReportGD/primary'
				},
				(res, err) => {
					primaryDocument = res;
				}
			...
			await upload(
				...
				{
					directory: '/investigationReportGD/secondary/images'
				},
				(res, err) => {
			...
			await upload(
				...
				{
					directory: '/investigationReportGD/secondary/documents'
				},
				(res, err) => {
			...
			await createInvestigationReportGD(
				{
					gd: data['gd_id'],
				...
				async (res, err) => {
					if (res) {
						await updateGDTimeline(
							{
								_id: data['gd_id']
							},
							{
								toPush: {
									documentType: 'Investigation Report GD',
									id: res.id,
									...
									<div className={classes.box}>
										<AccordionDetails>
											...
											<Typography className={classes.errorMSG}>
												{errors.gd_id && (
													<span>{errors.gd_id.message}</span>
												)}
											</Typography>
										</AccordionDetails>
										<AccordionDetails>
