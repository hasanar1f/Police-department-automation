...
import createPresidentAppeal from '../../axios/presidentAppeal/create';
	...
	const onSubmit = async data => {
		if (scanCopy.length < 1) {
			...
			await upload(
				...
				{
					directory: '/presidentAppeal/primary'
				},
				(res, err) => {
					primaryDocument = res;
				}
			...
			await createPresidentAppeal(
				{
					case: data['case_no'],
					date: data['appeal_date'],
					applicant: data['applicant_name'],
					application: data['appeal_details'],
				...
				async (res, err) => {
					if (res) {
						await updateCaseTimeline({
							...
							toPush: {
								documentType: 'President Appeal',
								id: res.id,
								recordedBy: JSON.parse(localStorage.getItem('officer'))._id
							}
						}, {
							Authorization: 'Officer ' + localStorage.getItem('token'),
							'Content-type': 'application/json'
			...
			reset_dict['case_no'] = '';
			reset_dict['appeal_date'] = new Date();
			reset_dict['applicant_name'] = '';

			reset_dict['appeal_details'] = '';
			setScanCopy([]);
								...
								<div className={classes.wrap}>
									<div className={classes.box}>
										...
										<AccordionDetails>
											...
											<Typography className={classes.errorMSG}>
												{errors.appeal_date && (
													<span>{errors.appeal_date.message}</span>
												)}
											</Typography>
										</AccordionDetails>
											...
											<Controller
												...
												render={({ field: { ref, ...field } }) => (
													<TextField
														...
														id='appeal_details'
														// label="ঘটনার বিবরণ"
														placeholder='আবেদনপত্র*'
														multiline
														variant='outlined'
														color='secondary'
														fullWidth
														rows={12}
														error={!!errors.appeal_details}
													/>
												)}
											/>
											<br />
...
export default PresidentAppealForm;
