...
const DivisionViz = () => {
		...
		await masterQuery(
			...
			(res, err) => {
				if (res) {
					...
					const divisionName = location.state.division_name;
					const dummyData = [];
					const dummyDataForDistrict = [];
					res.data01.map(div => {
						if (div.division === divisionName) {
							// console.log('here');
							let land = 0;
							let female_crime = 0;
							let other = 0;
							div.crime_wise.map(crime => {
								if (
									crime._id === 'land' ||
									crime._id === 'contract' ||
									crime._id === 'property' ||
					...
					res.data02.map(dist => {
						if (dist.division === divisionName) {
							let temp = {};
							temp['name'] = dist.district;
							temp['value'] = dist.total_crime_count;
							dummyDataForDistrict.push(temp);
						}
					});
					setCrimeWiseData(dummyData);
					setDistrictWiseData(dummyDataForDistrict);
					setLoading(false);
				} else {
					console.log(err);
				}
	...
	let colors = pickXRandomColors(districtWiseData.length);
	...
	const handleClickPie = obj => {
		console.log(obj);
		const districtName = obj.name;
		history.push({
			pathname: '/visualize/district',
			state: {
			...
			<Grid>
				...
				>
					...
					<Grid item xs={12} sm={5}>
						<Paper className={classes.paper}>
							<Typography variant="h5">
								{divisionLabel} বিভাগের অপরাধচিত্র
							</Typography>
							<br />
					...
					<Grid item xs={12} sm={5}>
						<Paper className={classes.paper}>
							<Typography variant="h5">
								{divisionLabel} বিভাগের জেলা সমূহ
							</Typography>
							<br />
